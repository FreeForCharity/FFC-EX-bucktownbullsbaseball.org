import type { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Privacy Policy',
  description: 'Privacy Policy for the Bucktown Bulls Baseball website.',
}

export default function PrivacyPolicy() {
  return (
    <div className="bg-cream">
      <article className="bb-container max-w-[760px] py-16 md:py-20 bb-prose">
        <p className="font-(family-name:--font-inter) text-[0.72rem] uppercase tracking-[0.22em] text-stone">
          Site Information
        </p>
        <h1 className="mt-3 font-(family-name:--font-bebas-neue) text-navy text-[2.6rem] md:text-[3rem] leading-none tracking-wide">
          Privacy Policy
        </h1>
        <p className="mt-3 text-stone font-(family-name:--font-inter) text-[0.9rem]">
          Effective date: May 2026
        </p>

        <h2 className="text-[1.6rem] mt-10 mb-3">What we collect</h2>
        <p>
          The Bucktown Bulls Baseball website (bucktownbullsbaseball.org) is a static information
          site. We do not run accounts, comments, e-commerce, newsletters, or tracking analytics on
          this site. We do not knowingly collect personal information from visitors.
        </p>

        <h2 className="text-[1.6rem] mt-8 mb-3">Hosting and server logs</h2>
        <p>
          This site is hosted by GitHub Pages on behalf of Free For Charity, a 501(c)(3) nonprofit
          that provides free hosting for nonprofit organizations. The hosting provider may keep
          standard server logs (IP address, browser, time of request) for the purpose of security
          and abuse prevention. The organization does not access or retain those logs.
        </p>

        <h2 className="text-[1.6rem] mt-8 mb-3">Cookies</h2>
        <p>
          This site does not set tracking cookies. Your browser may store a small amount of
          information used by the site itself (for example, remembering whether you have closed a
          menu), but none of it leaves your browser.
        </p>

        <h2 className="text-[1.6rem] mt-8 mb-3">Roster and team information</h2>
        <p>
          Information about individual players, teams, and families is not published on this site.
          It is shared directly with families through team communication channels run by the
          organization (text threads, team apps, and email between coaches and parents).
        </p>

        <h2 className="text-[1.6rem] mt-8 mb-3">Links to other sites</h2>
        <p>
          We may link to outside organizations and team services. Those sites have their own privacy
          practices, and we are not responsible for their content.
        </p>

        <h2 className="text-[1.6rem] mt-8 mb-3">Contact</h2>
        <p>
          Questions about this policy can be directed to{' '}
          <a
            href="mailto:info@bucktownbullsbaseball.org"
            className="underline underline-offset-4 hover:text-red"
          >
            info@bucktownbullsbaseball.org
          </a>
          .
        </p>
      </article>
    </div>
  )
}
